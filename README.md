...
- Port 8888 accessible in your firewall and router (or set `PORT`)
...
The server automatically finds Let's Encrypt certs at the standard path — no copying needed. Renew with `sudo certbot renew`. Guests connect at `https://yourname.duckdns.org:8888`.
...
Place the files alongside `nes-server.js`. The server detects them automatically. Browsers will show a security warning on first visit — click through to proceed. Not suitable for remote play.
...
```bash
PORT=9000 node nes-server.js   # use a different port
```
